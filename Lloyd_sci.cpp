...
double M_k_double(double x, double Mu, double dt)// drift aussi egale à l'esperance de la marginal
{
	return res = x + Mu * x*dt;
}

//function[res] = S_k_CEV(x, Nu, Delta, dt)// drift aussi egale à l'esperance de la marginal
double S_k_CEV(double x, double Nu, double Delta, double dt)
{
		return res = Sigma.*x*sqrt(dt);
}
//function[Grilles, Poids] = double Lloyd_1d(nb_quant, init, nb_iter)
std::vector <std::pair<double, double>>Lloyd_1d(int nb_quant, std::vector<double> init, int nb_iter)
{
	int compteur = 0;
	std::vector<double> Grilles = init;
	std::vector<double> Poids(nb_quant, 1);
	for (compteur = 1; compteur <= nb_iter; ++compteur)
	{
		std::vector<double> tmp_memoire = (cat(2, Grilles, %inf) + cat(2, -%inf, Grilles));
...
//function[G_t, P_t] = Lloyd_RMQ(G_moins, P_moins, Mu, Sigma, dt)// Recursive Marginal Quantization
	std::vector<std::pair<double, double> > Lloyd_RMQ(std::vector<double> G_moins,
													  std::vector<double> P_moins,
													  double Mu, double Sigma,double dt)
{
	int compteur = 0;
	int nb_quant = G_moins.size();
	std::vector<double> G_t = G_moins;
	std::vector<double> P_t (nb_quant,1);
